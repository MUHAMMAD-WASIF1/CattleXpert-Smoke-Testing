...
describe('Add New Risk and verify', () => {

    it('NAVIGATION TO System Setup > Health > Risk', () => {
       //Risk
    cy.get('#Item_COR48').click()
    cy.get('#Item_COR92').click()
    cy.get('#Item_COR104').click()
    ...
    it('Add New Rider/Treatment Person ', () => {
        ...
        cy.get('#txtRiskShortName').focus().type(randomString)
        

        cy.get('#btnSave').click()
        cy.wait(1000)
        cy.get('#popup_message').should('contain', 'Risk has been saved successfully')
        cy.wait(1000)
