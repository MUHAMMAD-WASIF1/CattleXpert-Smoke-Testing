...
describe('Add New Treatment Location and verify', () => {

    it('NAVIGATION TO System Setup > Health > Treatment Location', () => {
        //Treatment Location
    cy.get('#Item_COR48').click()
    cy.get('#Item_COR92').click()
    cy.get('#Item_COR149').click()
    cy.contains('#formcontainer > h1 > span:nth-child(2)','Treatment Location').should('be.visible')

    })

    it('Add New Treatment Location ', () => {
        ...
        cy.wait(1000)
        cy.get('#popup_message').should('contain', 'Treatment Location has been saved successfully')
        cy.wait(1000)
        cy.get('#popup_ok').click()
    })

    it('Verify Treatment Location', () => {

        cy.get('#jqgh_productGridView_CreatedDate').click()
        cy.wait(2000)
        cy.get('#jqgh_productGridView_CreatedDate').click()
        cy.wait(2000)
